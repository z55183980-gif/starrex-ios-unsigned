      ...
      <div class="task-header-card">
        <div class="task-header-top">
          <div class="header-left">
            <div class="title">{{ periodLabel }}投注任务</div>
  ...
  try {
    ...
    try {
      ...
      if (res.code === 0 && res.data) {
        periodBetAmount.value = parseFloat(res.data.periodBetAmount || res.data.todayBetAmount) || 0
        periodLabel.value = res.data.periodLabel || '每日'
        ...
        if (res.data.allLevels && res.data.allLevels.length > 0) {
          rewards.value = res.data.allLevels.map(item => ({
            ...
            currentProgress: periodBetAmount.value,
