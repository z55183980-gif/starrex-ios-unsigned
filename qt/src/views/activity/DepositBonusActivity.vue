<template>
  <div class="deposit-bonus-activity">
    <van-nav-bar
      :title="activityTitle"
      left-arrow
      fixed
      ...
      <div class="status-card">
        <div class="status-row">
          <div class="status-item">
            {{ displayLabel }} <span class="amount-gold">{{ displayAmount }}</span>
          </div>
          <div class="status-item right">
            已发放 <span class="amount-gold">{{ claimedAmount }}</span>
          ...
          <div class="table-body">
            ...
            >
              ...
              <div class="td action-col">
                <div 
                  class="action-btn"
                  :class="{ 'btn-claimed': item.isClaimed, 'btn-gray': !item.isClaimed }"
                >
                  {{ item.statusText }}
                </div>
              </div>
...
const periodDeposits = ref('0.00')
const latestDepositAmount = ref('0.00')
const claimedAmount = ref('0.00')
const displayAmount = computed(() => calcMode.value === 'cumulative' ? periodDeposits.value : latestDepositAmount.value)
const displayLabel = computed(() => calcMode.value === 'cumulative' ? '累计充值' : '单笔充值')
...
const loadData = async () => {
  ...
  try {
    ...
    if (res.code === 0 && res.data) {
      calcMode.value = res.data.calcMode || 'single'
      periodDeposits.value = res.data.periodDeposits || '0.00'
      latestDepositAmount.value = res.data.latestDepositAmount || '0.00'
      ...
      if (res.data.allLevels && res.data.allLevels.length > 0) {
        rewards.value = res.data.allLevels.map(item => ({
          ...
          statusText: item.isClaimed ? '已发放' : '未达标'
        }))
        
        const claimedTotal = rewards.value
          .filter(r => r.isClaimed)
          .reduce((sum, r) => sum + parseFloat(r.rewardAmount || 0), 0)
        claimedAmount.value = claimedTotal.toFixed(2)
      } else {
        claimedAmount.value = '0.00'
      }
    }
  } catch (error) {
  } finally {
